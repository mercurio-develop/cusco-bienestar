...
const TRIGGER_WORDS = ['VERIFICA', 'VERIFY', 'CUSCO BIENESTAR', 'VALLE', 'SI', 'SÍ', 'YES', 'OK', '1'];
...
function buildExploreUrl(business: { name: string; locationSlug: string | null; category: string }) {
  ...
  return `https://cusco-bienestar.com/explore/${loc}/${cat}?q=${encodeURIComponent(business.name)}`;
}
