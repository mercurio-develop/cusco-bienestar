...
import { Search, X, SlidersHorizontal, Navigation, MapPin, ChevronDown } from "lucide-react"
import posthog from 'posthog-js'
import { motion, AnimatePresence } from "framer-motion"
import { CATEGORIES, LOCATIONS, RADIUS_MARKS, DICTIONARIES } from "../constants"
...
export function ExploreSearch({ 
  ...
  const currentLoc = params.loc || ""
  const currentCatStr = params.cat || ""
  const isRadEnabled = params.rad !== null
  const currentRad = isRadEnabled ? params.rad! : 5
  const currentLat = params.lat?.toString() || ""
  const currentLng = params.lng?.toString() || ""
  ...
  const t = ((dict as any)?.explore) || DICTIONARIES.explore
  ...
  const navigateFilters = (loc?: string, cats?: string[], q?: string, rad?: number, lat?: string, lng?: string, sort?: string) => {
    setParams({
      q: q || null,
      loc: loc || null,
      cat: cats && cats.length > 0 ? cats.join(',') : null,
      rad: rad !== undefined ? rad : null,
      lat: lat ? parseFloat(lat) : null,
      lng: lng ? parseFloat(lng) : null,
      sort: sort && sort !== "recommended" ? sort : null,
      filter: showFilters ? "open" : null,
    }, { shallow: false })
  }
        ...
        {showFilters && (
          ...
          >
            <div className="flex flex-col gap-5 pt-2 pb-4 font-sans">
              ...
              {/* Location - Magazine / Airbnb Hybrid Pills */}
              <div className="flex flex-col gap-2">
                ...
                >
                  <span className="text-[11px] font-bold text-slate-600 uppercase tracking-[0.15em] transition-colors">{t.townSector}</span>
              ...
              {/* Categories - Magazine / Airbnb Hybrid */}
              <div className="flex flex-col gap-2">
                <button 
                  onClick={() => setShowCategories(!showCategories)}
                  className="flex items-center justify-between w-full p-2 -ml-2 rounded-xl hover:bg-slate-50 transition-colors group"
                >
                  <span className="text-[11px] font-bold text-slate-600 uppercase tracking-[0.15em] transition-colors">{t.categoriesLabel}</span>
                    ...
                    >
                      <div className="grid grid-cols-3 gap-2 pb-1">
                        ...
                        >
                          ...
                          <span className={cn("whitespace-nowrap", currentCats.length === 0 ? "text-[10px] font-black uppercase tracking-[0.15em] text-white/90" : "text-[9px] font-semibold tracking-tight")}>{t.all}</span>
                        </button>
                        {CATEGORIES.filter(cat => 
                          ...
                          return (
                            ...
                            >
                              ...
                              />
                              <span className={cn("whitespace-nowrap", isActive ? "text-[10px] font-black uppercase tracking-[0.15em] text-white/90" : "text-[9px] font-semibold tracking-tight")}>{t.categories[cat.value] || cat.label}</span>
                              {count > 0 && !isActive && (
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold text-slate-600 uppercase tracking-[0.15em]">{t.radiusSearch}</span>
                    <button
                  ...
                  {isRadEnabled && (
                    ...
                    >
                      ...
                      {isGettingLocation ? t.locating : (currentLat ? t.usingGps : t.myLocation)}
                    </button>
                  )}
                </div>
                <AnimatePresence>
