...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const professional = await prisma.business.findUnique({
    where: { slug },
    select: { name: true, category: true, tagline: true, imageUrl: true, rating: true, locationSlug: true },
  })

  if (!professional) return { title: "Not Found | Cusco Bienestar" }

  const locationName = professional.locationSlug
    ? professional.locationSlug.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")
    : "Sacred Valley"

  const title = `${professional.name} — ${professional.category} in ${locationName} | Cusco Bienestar`
  const description =
    professional.tagline ||
    `Connect with ${professional.name}, a verified wellness professional in ${locationName}.`
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${siteConfig.url}/professionals/${slug}`,
      images: professional.imageUrl ? [{ url: professional.imageUrl, width: 1200, height: 630 }] : [],
    },
  }
}
...
export default async function ProfessionalDetailPage({ params }: Props) {
  ...
  if (
    !professional ||
    !PROFESSIONAL_CATEGORIES.includes(professional.category?.toLowerCase() ?? "")
  ) {
    return notFound()
  }

  const description = professional.longDescription || professional.description

  // Build images array
  const images: string[] = []
  if (professional.imageUrl) images.push(professional.imageUrl)
  try {
    const heroImages = professional.heroImages ? JSON.parse(professional.heroImages) : []
  ...
  const whatsappMsg = encodeURIComponent(
    `Hello ${professional.name}! 👋 I saw your profile on Cusco Bienestar and I'd like to make a booking. When do you have availability?`
  )
  const whatsappHref = professional.whatsapp
    ? `https://wa.me/51${professional.whatsapp.replace(/\D/g, "")}?text=${whatsappMsg}`
    : null

  const instagramHandle = professional.instagramHandle?.replace("@", "")
  const instagramHref = instagramHandle ? `https://instagram.com/${instagramHandle}` : null

  const canContact = professional.isAsociado && whatsappHref

  return (
    <main className="min-h-screen bg-gray-50 pb-24 lg:pb-12">
      {/* Back button */}
      <div className="absolute top-4 left-4 z-50">
        <BackButton fallbackHref="/professionals" label="Professionals" className="bg-white/80 backdrop-blur-md shadow-sm" />
      </div>
      ...
      <section className="relative w-full h-[40vh] min-h-[280px] lg:h-[50vh] lg:max-w-6xl lg:mx-auto lg:rounded-b-3xl overflow-hidden bg-slate-200 shadow-sm">
        ...
        {professional.rating != null && (
          <div className="absolute -bottom-4 right-4 z-10 bg-white px-4 py-1.5 rounded-full shadow-md border border-slate-100 flex items-center gap-1 lg:hidden">
            ...
            <span className="font-medium text-sm text-slate-900">{professional.rating.toFixed(1)}</span>
            <span className="text-xs text-slate-500 ml-1">({professional.reviewsCount || 0})</span>
          </div>
        )}
      </section>
        ...
        <div className="lg:col-span-2">
          ...
          <section className="px-4 lg:px-0 pt-8 lg:pt-0 pb-6 bg-white lg:bg-transparent -mt-6 lg:mt-0 relative z-20 rounded-t-[2rem] lg:rounded-none">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-4 text-sm text-slate-600 mb-6">
              ...
              {professional.priceTier && (
                <>
                  <span>•</span>
                  <span className="font-medium">{professional.priceTier}</span>
                </>
              )}
              <span>•</span>
          ...
          {professional.services.length > 0 && (
            <section className="px-4 lg:px-0 py-8 bg-white lg:bg-transparent border-t border-slate-100 lg:border-0">
              <h2 className="text-xl lg:text-2xl font-serif text-slate-900 mb-6">Services</h2>
                    ...
                    <div className="flex-1">
                      ...
                      {service.durationMins && (
                        <div className="flex items-center gap-1 mt-2 text-slate-500">
                          ...
                          <span className="text-xs">{service.durationMins} min</span>
                        </div>
                      )}
                    </div>
            ...
            {professional.rating != null && (
              <div className="flex flex-col items-center justify-center pb-6 border-b border-slate-100">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="font-bold text-2xl text-slate-900">{professional.rating.toFixed(1)}</span>
                </div>
                <span className="text-sm font-medium text-slate-500">
                  {professional.reviewsCount || 0} reviews
                </span>
              </div>
            )}
